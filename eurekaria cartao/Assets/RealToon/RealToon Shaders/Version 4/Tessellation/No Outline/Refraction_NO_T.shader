...
Shader "RealToon/Version 4/Tessellation/No Outline/Refraction" {
    Properties {

        [Header((RealToon V4.1.2))]
    	[Header((Tessellation x No Outline x Refraction))]

    	[Space(20)][Enum(Off,2,On,0)] _DoubleSided("Double Sided", int) = 2

    	[Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Refraction))]_RefractionIntensity ("Intensity", Range(0, 1)) = 0.1

        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Texture Color))]_MainColor ("Main Color", Color) = (0.8014706,0.8014706,0.8014706,1)
        ...
        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Shadow))]_ShadowAddLight ("Add Light", Float ) = 1

        [Space(8)][Header(Self Shadow)]_SelfShadowIntensity ("Intensity", Range(0, 1)) = 0
        ...
        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Lighting))][MaterialToggle] _UseGILighting ("Use GI Lighting", Float ) = 0
        ...
        [MaterialToggle] _FresnelOnLight ("On Light", Float ) = 0
        [MaterialToggle] _FresnelOnShadowDark ("On Shadow/Dark", Float ) = 0

        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Tessellation))]_TessellationSmoothness ("Smooth Edge", Range(0, 1)) = 0.5
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_3255 = (tex2D( _GrabTexture, lerp(sceneUVs.rg,reflect(float3(sceneUVs.rg,0.0),normalDirection).rg,lerp(0.0,(-0.2),_RefractionIntensity))).rgb*_MainColor.rgb);
                float4 _Texture_var = tex2D(_Texture,TRANSFORM_TEX(i.uv0, _Texture));
                float3 node_5260 = lerp(node_3255,lerp(node_3255,_Texture_var.rgb,_Texture_var.a),_TextureIntensity);
                ...
                float3 node_8500 = lerp(node_5260,lerp(node_5260,lerp(lerp(_FReflection_var.rgb,lerp(node_5260,_FReflection_var.rgb,_FReflection_var.rgb),_ReflectionBlendToMainTexture),node_5260,(1.0 - _MaskReflection_var.r)),_ReflectionIntensity),_UseFReflection);
                float3 node_5080 = (node_8500*(0.8*_ShadowAddLight));
                ...
                float node_4465 = 0.2;
                float node_7903 = 0.3;
                float3 _UseGILighting_var = lerp( UNITY_LIGHTMODEL_AMBIENT.rgb, GI_Light_Probe( lerp(float3(node_6160,node_6160,node_6160),node_8500,lerp( (normalDirection*_GIShadeSize), float3(smoothstep( float2(node_4465,node_4465), float2(node_7903,node_7903), (normalDirection.rb*_GIShadeSize) ),0.0), _GIFlatShade )) ), _UseGILighting );
                ...
                float3 node_4224 = (lerp(lerp(lerp(node_5260,node_5080,(1.0 - _EnableLightFalloff_var)),node_5080,(1.0 - (node_6482*node_1068))),node_5080,IFs( 0.0 , 1.0 ))*(lerp(_UseGILighting_var,(_UseGILighting_var+_LightColor0.rgb),_DirectionalLightAffectShadow)+lerp( 0.0, node_137, _FresnelOnShadowDark )));
                float4 _MaskSelfLit_var = tex2D(_MaskSelfLit,TRANSFORM_TEX(i.uv0, _MaskSelfLit));
                float3 node_4559 = lerp(node_4224,lerp(node_4224,((node_5260*_SelfLitColor.rgb*lerp( 1.0, node_5260, _SelfLitHighContrast ))*_SelfLitPower),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                ...
                float node_3815 = 0.0;
                float3 node_7256 = ((((node_8500*(_EnableLightFalloff_var*node_1068))*node_6482)+(lerp(float3(node_7581,node_7581,node_7581),lerp(float3(node_3179,node_3179,node_3179),(node_3179*_GlossColor.rgb),2.22),_MaskGloss_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(float3(node_3815,node_3815,node_3815),lerp( node_3815, node_137, _FresnelOnLight ),node_2697))*_LightColor0.rgb*lerp(1.0,0.3,_DirectionalLightAffectShadow));
                float node_1002 = 0.0;
                float3 node_714 = lerp(node_7256,lerp(node_7256,float3(node_1002,node_1002,node_1002),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float3 finalColor = emissive + lerp(lerp(lerp(node_714,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_714,_UseFReflection),dot(lerp(lerp(node_714,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_714,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_2058 = 0.0;
                float node_3161 = (1.0 - _Saturation);
                float3 finalColor = lerp(lerp(lerp(node_714,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_714,_UseFReflection),dot(lerp(lerp(node_714,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_714,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
