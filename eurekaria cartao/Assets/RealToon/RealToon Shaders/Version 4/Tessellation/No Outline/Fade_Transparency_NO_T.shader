...
Shader "RealToon/Version 4/Tessellation/No Outline/Fade Transparency" {
    Properties {

        [Header((RealToon V4.1.2))]
    	[Header((Tessellation x No Outline x Fade Transparency))]
        ...
        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Opacity))]_Opacity ("Opacity", Range(0, 1)) = 1
        _MaskTransperancy ("Mask Transperancy", 2D) = "black" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _MaskTransperancy; uniform float4 _MaskTransperancy_ST;
            struct VertexInput {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                clip(saturate(( 0.74 > 0.5 ? (1.0-(1.0-2.0*(0.74-0.5))*(1.0-_Texture_var.a)) : (2.0*0.74*_Texture_var.a) )) - 0.5);
                ...
                float4 node_2021_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_2021_p = lerp(float4(float4(_LightColor0.rgb,0.0).zy, node_2021_k.wz), float4(float4(_LightColor0.rgb,0.0).yz, node_2021_k.xy), step(float4(_LightColor0.rgb,0.0).z, float4(_LightColor0.rgb,0.0).y));
                float4 node_2021_q = lerp(float4(node_2021_p.xyw, float4(_LightColor0.rgb,0.0).x), float4(float4(_LightColor0.rgb,0.0).x, node_2021_p.yzx), step(node_2021_p.x, float4(_LightColor0.rgb,0.0).x));
                float node_2021_d = node_2021_q.x - min(node_2021_q.w, node_2021_q.y);
                float node_2021_e = 1.0e-10;
                float3 node_2021 = float3(abs(node_2021_q.z + (node_2021_q.w - node_2021_q.y) / (6.0 * node_2021_d + node_2021_e)), node_2021_d / (node_2021_q.x + node_2021_e), node_2021_q.x);;
                float node_6160 = 0.0;
                float node_4465 = 0.2;
                float node_7903 = 0.3;
                ...
                float3 node_4224 = (lerp(lerp(node_5080,lerp(lerp(node_4607,node_5080,(1.0 - _EnableLightFalloff_var)),node_5080,(1.0 - (node_6482*node_1068))),saturate((node_2021.b+node_2021.b+0.05))),node_5080,IFs( 0.0 , 1.0 ))*(lerp(_UseGILighting_var,(_UseGILighting_var+_LightColor0.rgb),_DirectionalLightAffectShadow)+lerp( 0.0, lerp(node_137,float3(node_8889,node_8889,node_8889),node_2697), _FresnelOnShadowDark )));
                float4 _MaskSelfLit_var = tex2D(_MaskSelfLit,TRANSFORM_TEX(i.uv0, _MaskSelfLit));
                float3 node_9542 = lerp(node_4224,lerp(node_4224,((node_4607*_SelfLitColor.rgb*lerp( 1.0, node_4607, _SelfLitHighContrast ))*_SelfLitPower),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                ...
                float3 emissive = lerp(lerp(lerp(node_9542,lerp(node_9542,lerp(node_155,lerp(node_9542,node_155,node_155),_ReflectionBlendToMainTexture),_MaskReflection_var.r),_ReflectionIntensity),node_9542,_UseFReflection),dot(lerp(lerp(node_9542,lerp(node_9542,lerp(node_155,lerp(node_9542,node_155,node_155),_ReflectionBlendToMainTexture),_MaskReflection_var.r),_ReflectionIntensity),node_9542,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
                float node_7581 = 0.0;
                float node_9744 = pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(-2,15,_Glossiness)));
                float node_9905_ang = _GlossTextureRotate;
                float node_9905_spd = 1.0;
                float node_9905_cos = cos(node_9905_spd*node_9905_ang);
                float node_9905_sin = sin(node_9905_spd*node_9905_ang);
                float2 node_9905_piv = float2(0.5,0.5);
                ...
                float2 node_9905 = (mul(float2((-1*node_1696.r),node_1696.g)-node_9905_piv,float2x2( node_9905_cos, -node_9905_sin, node_9905_sin, node_9905_cos))+node_9905_piv);
                float2 node_4034 = (1.0 - (node_9905*0.5+0.5));
                ...
                float node_2886 = 0.0;
                float3 node_8605 = lerp(node_7256,lerp(node_7256,float3(node_2886,node_2886,node_2886),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float3 finalColor = emissive + lerp(lerp(lerp(node_8605,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_8605,_UseFReflection),dot(lerp(lerp(node_8605,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_8605,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
                float4 _MaskTransperancy_var = tex2D(_MaskTransperancy,TRANSFORM_TEX(i.uv0, _MaskTransperancy));
                fixed4 finalRGBA = fixed4(finalColor,lerp((_Texture_var.a*_MaskTransperancy_var.r),_Texture_var.a,_Opacity));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_2058 = 0.0;
                float node_3161 = (1.0 - _Saturation);
                float3 finalColor = lerp(lerp(lerp(node_8605,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_8605,_UseFReflection),dot(lerp(lerp(node_8605,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_8605,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
                float4 _MaskTransperancy_var = tex2D(_MaskTransperancy,TRANSFORM_TEX(i.uv0, _MaskTransperancy));
                fixed4 finalRGBA = fixed4(finalColor * lerp((_Texture_var.a*_MaskTransperancy_var.r),_Texture_var.a,_Opacity),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
