...
Shader "RealToon/Version 4/Tessellation/No Outline/Cutout" {
    Properties {

        [Header((RealToon V4.1.2))]
    	[Header((Tessellation x No Outline x Cutout))]
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                clip(lerp( ((_SecondaryCutout_var.r*_Texture_var.r)+lerp(0.5,(-1.0),_Cutout)), saturate(( (1.0 - _Cutout) > 0.5 ? (1.0-(1.0-2.0*((1.0 - _Cutout)-0.5))*(1.0-(_Texture_var.a*_SecondaryCutout_var.r))) : (2.0*(1.0 - _Cutout)*(_Texture_var.a*_SecondaryCutout_var.r)) )), _AlphaBaseCutout ) - 0.5);
                ...
                float4 node_8988_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_8988_p = lerp(float4(float4(_LightColor0.rgb,0.0).zy, node_8988_k.wz), float4(float4(_LightColor0.rgb,0.0).yz, node_8988_k.xy), step(float4(_LightColor0.rgb,0.0).z, float4(_LightColor0.rgb,0.0).y));
                float4 node_8988_q = lerp(float4(node_8988_p.xyw, float4(_LightColor0.rgb,0.0).x), float4(float4(_LightColor0.rgb,0.0).x, node_8988_p.yzx), step(node_8988_p.x, float4(_LightColor0.rgb,0.0).x));
                float node_8988_d = node_8988_q.x - min(node_8988_q.w, node_8988_q.y);
                float node_8988_e = 1.0e-10;
                float3 node_8988 = float3(abs(node_8988_q.z + (node_8988_q.w - node_8988_q.y) / (6.0 * node_8988_d + node_8988_e)), node_8988_d / (node_8988_q.x + node_8988_e), node_8988_q.x);;
                float node_6160 = 0.0;
                float node_4465 = 0.2;
                float node_7903 = 0.3;
                ...
                float3 node_4224 = (lerp(lerp(node_5080,lerp(lerp(node_4607,node_5080,(1.0 - _EnableLightFalloff_var)),node_5080,(1.0 - (node_6482*node_1068))),saturate((node_8988.b+node_8988.b+0.05))),node_5080,IFs( 0.0 , 1.0 ))*(lerp(_UseGILighting_var,(_UseGILighting_var+_LightColor0.rgb),_DirectionalLightAffectShadow)+lerp( 0.0, lerp(node_137,float3(node_8889,node_8889,node_8889),node_2697), _FresnelOnShadowDark )));
                float4 _MaskSelfLit_var = tex2D(_MaskSelfLit,TRANSFORM_TEX(i.uv0, _MaskSelfLit));
                float3 node_4559 = lerp(node_4224,lerp(node_4224,((node_4607*_SelfLitColor.rgb*lerp( 1.0, node_4607, _SelfLitHighContrast ))*_SelfLitPower),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                ...
                float node_285_ang = _GlossTextureRotate;
                float node_285_spd = 1.0;
                float node_285_cos = cos(node_285_spd*node_285_ang);
                float node_285_sin = sin(node_285_spd*node_285_ang);
                float2 node_285_piv = float2(0.5,0.5);
                ...
                float2 node_285 = (mul(float2((-1*node_1696.r),node_1696.g)-node_285_piv,float2x2( node_285_cos, -node_285_sin, node_285_sin, node_285_cos))+node_285_piv);
                float2 node_4034 = (1.0 - (node_285*0.5+0.5));
                ...
                float node_1275 = 0.0;
                float3 node_2481 = lerp(node_7256,lerp(node_7256,float3(node_1275,node_1275,node_1275),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float3 finalColor = emissive + lerp(lerp(lerp(node_2481,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2481,_UseFReflection),dot(lerp(lerp(node_2481,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2481,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_2058 = 0.0;
                float node_3161 = (1.0 - _Saturation);
                float3 finalColor = lerp(lerp(lerp(node_2481,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2481,_UseFReflection),dot(lerp(lerp(node_2481,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2481,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
