...
Shader "RealToon/Version 4/Tessellation/Outline/Default" {
    Properties {

        [Header((RealToon V4.1.2))]
    	[Header((Tessellation x Outline x Default))]
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_8682 = _Time;
                float node_5371_ang = node_8682.g;
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_5909_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_5909_p = lerp(float4(float4(_LightColor0.rgb,0.0).zy, node_5909_k.wz), float4(float4(_LightColor0.rgb,0.0).yz, node_5909_k.xy), step(float4(_LightColor0.rgb,0.0).z, float4(_LightColor0.rgb,0.0).y));
                float4 node_5909_q = lerp(float4(node_5909_p.xyw, float4(_LightColor0.rgb,0.0).x), float4(float4(_LightColor0.rgb,0.0).x, node_5909_p.yzx), step(node_5909_p.x, float4(_LightColor0.rgb,0.0).x));
                float node_5909_d = node_5909_q.x - min(node_5909_q.w, node_5909_q.y);
                float node_5909_e = 1.0e-10;
                float3 node_5909 = float3(abs(node_5909_q.z + (node_5909_q.w - node_5909_q.y) / (6.0 * node_5909_d + node_5909_e)), node_5909_d / (node_5909_q.x + node_5909_e), node_5909_q.x);;
                float node_6160 = 0.0;
                float node_4465 = 0.2;
                float node_7903 = 0.3;
                ...
                float node_798_ang = _GlossTextureRotate;
                float node_798_spd = 1.0;
                float node_798_cos = cos(node_798_spd*node_798_ang);
                float node_798_sin = sin(node_798_spd*node_798_ang);
                float2 node_798_piv = float2(0.5,0.5);
                ...
                float2 node_798 = (mul(float2((-1*node_1696.r),node_1696.g)-node_798_piv,float2x2( node_798_cos, -node_798_sin, node_798_sin, node_798_cos))+node_798_piv);
                float2 node_4034 = (1.0 - (node_798*0.5+0.5));
                ...
                float node_2922 = 0.0;
                float3 node_2106 = lerp(node_7256,lerp(node_7256,float3(node_2922,node_2922,node_2922),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float3 finalColor = emissive + lerp(lerp(lerp(node_2106,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2106,_UseFReflection),dot(lerp(lerp(node_2106,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2106,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_2058 = 0.0;
                float node_3161 = (1.0 - _Saturation);
                float3 finalColor = lerp(lerp(lerp(node_2106,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2106,_UseFReflection),dot(lerp(lerp(node_2106,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_2106,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
