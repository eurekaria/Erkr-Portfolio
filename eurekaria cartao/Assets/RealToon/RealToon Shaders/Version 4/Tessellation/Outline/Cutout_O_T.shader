...
Shader "RealToon/Version 4/Tessellation/Outline/Cutout" {
    Properties {

        [Header((RealToon V4.1.2))]
    	[Header((Tessellation x Outline x Cutout))]
        ...
        _MainColor ("Main Color", Color) = (0.8014706,0.8014706,0.8014706,1)
        ...
        [Space(8)][Enum(VNormal,0,Origin,1)] _OutlineMode("Outline Mode", int) = 0
        ...
        [Space(10)][Header(xxxxxxxxxxxxxxxx)][Header((Gloss))]_GlossIntensity ("Gloss Intensity", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            #pragma only_renderers d3d11 glcore gles3 metal d3d11_9x xboxone ps4
            #pragma target 4.6
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_5371_ang = node_1635.g;
                ...
                _OM = lerp(v.normal,normalize(v.vertex),_OutlineMode);
                o.pos = UnityObjectToClipPos( float4(v.vertex.xyz + _OM*(lerp(1.0,node_7493,_OutlineNoiseIntensity)*_OutlineWidth),1) );
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 _SecondaryCutout_var = tex2D(_SecondaryCutout,TRANSFORM_TEX(i.uv0, _SecondaryCutout));
                clip(lerp( ((_Texture_var.r*_SecondaryCutout_var.r)+lerp(0.5,(-1.0),_Cutout)), saturate(( (1.0 - _Cutout) > 0.5 ? (1.0-(1.0-2.0*((1.0 - _Cutout)-0.5))*(1.0-(_Texture_var.a*_SecondaryCutout_var.r))) : (2.0*(1.0 - _Cutout)*(_Texture_var.a*_SecondaryCutout_var.r)) )), _AlphaBaseCutout ) - 0.5);
        ...
        Pass {
            ...
            uniform fixed _GlossTextureFollowLight;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_5080 = ((lerp(node_487,lerp(float3(node_6497,node_6497,node_6497),node_487,_ShadowPTexture_var.r),_ShadowPTextureIntensity)*_ShadowOverrideColor.rgb)*(0.8*_ShadowAddLight));
                float _EnableLightFalloff_var = lerp( step(0.001,attenuation), attenuation, _EnableLightFalloff ); // Light Falloff Selection
                float node_3448 = 1.0;
                ...
                float node_2724 = lerp(1.0,saturate(round( 0.5*((_ShadowTTexture_var.r*(1.0 - _ShadowTShadowSize)) + saturate((lerp( node_573, (attenuation*node_573), _LightFalloffAffectShadowT )*_ShadowTLightSize))))),_ShadowTIntensity);
                float node_9545 = smoothstep( lerp(0.3,0.899,_SelfShadowHardness), 0.9, (node_573*lerp(2.8,0.79,_SelfShadowSize)) ); // Converted hard edge
                float node_6482 = lerp(lerp( node_3448, node_2724, _ShowShadowTOnSelfShadow ),lerp( node_3448, node_2724, _ShowShadowTOnLight ),node_9545);
                float node_5274 = 1.0;
                ...
                float4 node_4284_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_4284_p = lerp(float4(float4(_LightColor0.rgb,0.0).zy, node_4284_k.wz), float4(float4(_LightColor0.rgb,0.0).yz, node_4284_k.xy), step(float4(_LightColor0.rgb,0.0).z, float4(_LightColor0.rgb,0.0).y));
                float4 node_4284_q = lerp(float4(node_4284_p.xyw, float4(_LightColor0.rgb,0.0).x), float4(float4(_LightColor0.rgb,0.0).x, node_4284_p.yzx), step(node_4284_p.x, float4(_LightColor0.rgb,0.0).x));
                float node_4284_d = node_4284_q.x - min(node_4284_q.w, node_4284_q.y);
                float node_4284_e = 1.0e-10;
                float3 node_4284 = float3(abs(node_4284_q.z + (node_4284_q.w - node_4284_q.y) / (6.0 * node_4284_d + node_4284_e)), node_4284_d / (node_4284_q.x + node_4284_e), node_4284_q.x);;
                float node_6160 = 0.0;
                float node_4465 = 0.2;
                float node_7903 = 0.3;
                ...
                float3 node_137 = (((lerp( node_6003, smoothstep( 0.38, 0.4, node_6003 ), _HardEdgeFresnel )*1.0)*_FresnelColor.rgb)*lerp(0.0,10.0,_FresnelIntensity)); // Fresnel
                float node_8889 = 0.0;
                float node_2697 = (_EnableLightFalloff_var*node_9545);
                float3 node_4224 = (lerp(lerp(node_5080,lerp(lerp(node_4607,node_5080,(1.0 - _EnableLightFalloff_var)),node_5080,(1.0 - (node_6482*node_1068))),saturate((node_4284.b+node_4284.b+0.05))),node_5080,IFs( 0.0 , 1.0 ))*(lerp(_UseGILighting_var,(_UseGILighting_var+_LightColor0.rgb),_DirectionalLightAffectShadow)+lerp( 0.0, lerp(node_137,float3(node_8889,node_8889,node_8889),node_2697), _FresnelOnShadowDark )));
                ...
                float node_2302_ang = _GlossTextureRotate;
                float node_2302_spd = 1.0;
                float node_2302_cos = cos(node_2302_spd*node_2302_ang);
                float node_2302_sin = sin(node_2302_spd*node_2302_ang);
                float2 node_2302_piv = float2(0.5,0.5);
                ...
                float2 node_2302 = (mul(float2((-1*node_1696.r),node_1696.g)-node_2302_piv,float2x2( node_2302_cos, -node_2302_sin, node_2302_sin, node_2302_cos))+node_2302_piv);
                float2 node_4034 = (1.0 - (node_2302*0.5+0.5));
                ...
                float node_3815 = 0.0;
                float3 node_8001 = (((node_8500*(_EnableLightFalloff_var*node_1068))*node_6482)+(lerp(float3(node_7581,node_7581,node_7581),lerp( lerp(float3(node_3179,node_3179,node_3179),(node_3179*_GlossColor.rgb),2.22), lerp(float3(node_7359,node_7359,node_7359),_Texture_var.rgb,node_3179), _MainTextureColorGloss ),_MaskGloss_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(float3(node_3815,node_3815,node_3815),lerp( node_3815, node_137, _FresnelOnLight ),node_2697));
                float3 node_7256 = (lerp(lerp((node_8001*_AdjustLightPointSpotlight),dot((node_8001*_AdjustLightPointSpotlight),float3(0.3,0.59,0.11)),(1.0 - _SaturationPointSpotlight)),(node_8001*_AdjustLightDirectionalLight*lerp(1.0,0.3,_DirectionalLightAffectShadow)),IFs2( 0.0 , 1.0 ))*_LightColor0.rgb); // Attenuate and Color
                float node_3759 = 0.0;
                float3 node_7361 = lerp(node_7256,lerp(node_7256,float3(node_3759,node_3759,node_3759),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float3 finalColor = emissive + lerp(lerp(lerp(node_7361,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_7361,_UseFReflection),dot(lerp(lerp(node_7361,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_7361,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_5274 = 1.0;
                float node_573 = 0.5*dot(lerp( lightDirection, viewDirection, _SelfShadowatViewDirection ),normalDirection)+0.5; 
                ...
                float3 node_137 = (((lerp( node_6003, smoothstep( 0.38, 0.4, node_6003 ), _HardEdgeFresnel )*1.0)*_FresnelColor.rgb)*lerp(0.0,10.0,_FresnelIntensity)); // Fresnel
                float3 node_8001 = (((node_8500*(_EnableLightFalloff_var*node_1068))*node_6482)+(lerp(float3(node_7581,node_7581,node_7581),lerp( lerp(float3(node_3179,node_3179,node_3179),(node_3179*_GlossColor.rgb),2.22), lerp(float3(node_7359,node_7359,node_7359),_Texture_var.rgb,node_3179), _MainTextureColorGloss ),_MaskGloss_var.rgb)*lerp((node_8388_if_leA*node_2583)+(node_8388_if_leB*_GlossIntensity),_GlossIntensity,node_8388_if_leA*node_8388_if_leB))+lerp(float3(node_3815,node_3815,node_3815),lerp( node_3815, node_137, _FresnelOnLight ),node_2697)); 
                float3 node_7256 = (lerp(lerp((node_8001*_AdjustLightPointSpotlight),dot((node_8001*_AdjustLightPointSpotlight),float3(0.3,0.59,0.11)),(1.0 - _SaturationPointSpotlight)),(node_8001*_AdjustLightDirectionalLight*lerp(1.0,0.3,_DirectionalLightAffectShadow)),IFs2( 0.0 , 1.0 ))*_LightColor0.rgb); 
                float node_3759 = 0.0;
                float4 _MaskSelfLit_var = tex2D(_MaskSelfLit,TRANSFORM_TEX(i.uv0, _MaskSelfLit));
                float3 node_7361 = lerp(node_7256,lerp(node_7256,float3(node_3759,node_3759,node_3759),_MaskSelfLit_var.rgb),_SelfLitIntensity);
                float node_2058 = 0.0;
                float node_3161 = (1.0 - _Saturation);
                float3 finalColor = lerp(lerp(lerp(node_7361,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_7361,_UseFReflection),dot(lerp(lerp(node_7361,float3(node_2058,node_2058,node_2058),_ReflectionIntensity),node_7361,_UseFReflection),float3(0.3,0.59,0.11)),node_3161);
